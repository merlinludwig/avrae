...
{{ vArgs = argparse(ARGS) }}
{{ vOut = ['title','desc',[],f"COMMAND USED: !dogtime {' '.join(vIn)}"] }}
{{ nl = '\n' }}
{{ tb = '\t' }}
{{ vGactivity = load_json(get_gvar('3b1448cc-080b-499c-885f-d68eb1730ba7')) }}
{{ vResults = [] }}
...
{{ vOut[1] = f'{vGactivity[vIn1].desc}{nl+nl}{vGactivity[vIn1].resources}{nl+nl}{vGactivity[vIn1].resolution}{nl+nl}{vGactivity[vIn1].usage}' }}
...
[
	...
	"resolution":"After a workweek of carousing, a character stands to make contacts within the selected social class.\nA Charisma(Persuasion) check must be made. 1-5: one hostile contact, 6-10: no new contacts, 11-15: one allied contact, 16-20: two allied contacts, 21+: three allied contacts",
